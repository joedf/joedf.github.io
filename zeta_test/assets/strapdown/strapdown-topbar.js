// strapdown-topbar.js v1.6.4 ZETA test 01
// by Joe DF, Released under MIT License.
// Revision date: 2017-10-29 9:27:27 PM
...
var show_themeswitcher = true;
var strapdown_store = new Persist.Store('strapdown', { swf_path: '/persist.swf' });
var strapdown_themes = ['chaitin', "cerulean", "cosmo", "cyborg", "darkly", "flatly", "journal", "lumen", "paper", "readable", "sandstone", "simplex", "slate", "spacelab", "superhero", "united", "yeti"];
...
;(function(){
	...
	var topbar = document.getElementsByClassName('navbar')[0].getElementsByTagName('ul')[0];
	document.getElementsByClassName('navbar')[0].getElementsByClassName('container')[0].setAttribute('id','topbar');
	...
	navheader.innerHTML = '<div id="topbar_logo" style="display:inline-block;float:left;"></div>' + navheader.innerHTML;
	...
	css.type = "text/css";
	css.innerHTML = '.topbar-item,.headline-menu{font-size:14px!important}.topbar-item{text-align:right}'
				  //+ '@media(min-width:980px){.nav>li>a{display:inline!important;padding:0!important}}'
				  + '#strapdown-toc{overflow-x: hidden;}'
				  ...
				  + 	'.topbar-item,.dropdown-toggle{text-align:'+calign+'}'
				  + 	'.dropdown{left:unset;right:0;max-width:280px}' //fix (most) wide menus from going out of the screen on mobile
				  +		'.dropdown-menu > li {position: relative;z-index: 2000;}'
				  ...
				  + '.dropdown > ul{max-width:320px;overflow-y:auto;max-height:550px;list-style:none !important;margin:0 !important}';
	...
	//zeta css compatfix
	document.getElementsByClassName('navbar-collapse')[0].setAttribute('id','navbar-main');
	...
	*/
	...
	//add nbar_toggle to zeta's 'hamburger' menu
	document.getElementsByClassName('icon-bar')[0].parentNode.setAttribute('onclick','nbar_toggle()');
	...
	for (var i = 0; i < topbar_items.length; i++) {
		...
		if (item.parentNode == topbar_tag) { // Process only top-level tags
			if (item.tagName.toUpperCase() == "MENU") { // Process Menu-type tags
				...
				content = content + '<li class="dropdown"><a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" onclick="dmenu_toggle(this,1)">'+menu_name+'<span class="caret"></span></a><ul class="dropdown-menu">';
				...
				content = content + '<li class="dropdown"><a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" onclick="dmenu_toggle(this,1)">'+toc_name+'<span class="caret"></span></a><ul class="dropdown-menu" id="strapdown-toc">';
				// Finalize TOC preparation
				content = content + '</ul></li>';
			} else { // Otherwise, process as simple <item> tag
				content = content + '<li class="topbar-item">' + item.innerHTML + '</li>';
			}
		}
	}
	...
	//theming functions
	var themercode = document.createElement("script");
	themercode.innerHTML = 'function set_theme(theme="cosmo"){'
						   + 	'theme = theme.toLowerCase().trim();'
						   + 	'if (!strapdown_themes.includes(theme)){return -1;}'
						   + 	'if (typeof base == "undefined"){base = getScriptBase("/strapdown");}'
						   + 	'if (typeof strapdown_store == "undefined"){'
						   +		'strapdown_store = new Persist.Store("strapdown", { swf_path: "/persist.swf" });'
						   +	'}'
						   + 	'strapdown_store.set("theme", theme);'
						   + 	'return upsertTheme(base, theme);'
						   + '};';
	document.body.appendChild(themercode);

	//hack the zeta theme-switcher menu to work with this plugin
	if (show_themeswitcher) {
		topbar.innerHTML += content;
		document.getElementById('theme').parentNode.getElementsByTagName('a')[0].setAttribute('href','javascript:;');
		document.getElementById('theme').parentNode.getElementsByTagName('a')[0].setAttribute('onclick','dmenu_toggle(this,1)');
		var themeEl = document.getElementById('theme');
		var theme_lis = document.getElementById('theme').getElementsByTagName('li');
		for (var i = 0; i < theme_lis.length; i++) {
			//following is forked from zeta
			addEvent(theme_lis[i], 'click', function (e) {
				var new_theme = e.target.innerHTML.toLowerCase();
				set_theme(new_theme);
				var actives = document.querySelectorAll("li.active");
				[].forEach.call(actives, function(ele){ele.className = "";})
	...
	window.onload = function() { //wait for window to load for window.getComputedStyle
		...
		for (var i = 1; i < 7; i++) {
			...
			if (!!h_e) {
				...
				var h_tb = parseInt(document.getElementById('topbar').scrollHeight,10);

				var tb_offset = (h_tb>=43)?(h_tb - 43):(0);
				
				_aoffset = h_fs+topbar_h+(h_oh-h_lh)+tb_offset+4;
				_moffset = h_fs+7+mfixed_offset+(tb_offset*2);
				haligh_css = haligh_css + '.h'+i+'_anchor{position:relative;display:inline-block;top:-'+(_aoffset)+'px;}';
				haligh_css = haligh_css + '@media(max-width:767px){.h'+i+'_anchor{position:relative;display:'+mfx_bType+';top:-'+(_moffset)+'px}}';
			}
		}
		var css = document.createElement("style");
		css.type = "text/css";
		css.innerHTML = 'h1,h2,h3,h4,h5,h6{display:inline-block}'
					  ...
					  + '.header-link{display:none !important;}' //hides zeta's header para links
					  + haligh_css;
		document.body.appendChild(css);
	}
})();